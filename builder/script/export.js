...
exportControls.copy.addEventListener("click", () => {
    navigator.clipboard.writeText(content.innerText).then(() => {
        ...
        console.error(err)
        alert("there was a problem copying your article :/ (check the console for more information)")
    });
})
...
exportControls.stash.addEventListener("click", () => {
    ...
    for (const file in fileList) {
        ...
        button.onclick = () => {
            let confirmation = confirm("are you sure you want to overwrite " + file + "?")
            if (!confirmation) {
                alert("ok cancelled")
                return
            }
...
stashControls.nameSubmit.addEventListener("click", () => {
    ...
    if (fileList[name]) {
        let confirmation = confirm("are you sure you want to overwrite " + name + "?")
        if (!confirmation) {
            alert("ok cancelled")
            return
        }
